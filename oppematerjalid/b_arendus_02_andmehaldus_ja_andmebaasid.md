...
### 2.1.1 Andmed ja **i** nformatsioon
...
**Andmebaas (**_ **database** _**)** **on omavahel seotud ja süstematiseeritud andmete kogum****.**
...
Arvutiprogrammi, milline hoiab andmeid ja võtab vastu kasutajate päringuid ning väljastab päringu põhjal vajalikud andmed (kui leiduvad!) vajalikul kujul nimetatakse **andmebaasi haldussüsteemiks** (**ABHS)**_(_[database management system](http://en.wikipedia.org/wiki/Database_management_system)_(DBMS)_).
...
- atomaarsus (_Atomicity\_\_)_ - ülekanne peab olema jagamatu, saabudes kehtivalt (juriidiliselt) lõpetatuna (soorita tehing=_commit_) ja vastasel korral tühistatuna (jäta tehing ära=_rollback\_\_)_, viimane taastab andmete esialgse seisu (seis enne tehingu alustamist), mis loogikaterminites tähendab kõik või mitte midagi
...
- eraldatus (_Isolation_) – üheaegset juurdepääsu _(concurrency_) andmetele erinevate protsesside poolt peab reguleerima, et vältida konflikte ja ABHS väärastumise/rippuma jäämise (_deadlock_) situatsioonide vältimiseks
...
1. Kas Statistikaameti poolt väljastatud näitaja &quot;keskmine eluiga&quot; on informatsioon?

A) jah X

B) ei

## 2.2 Andmebaasi struktuur
...
Iga OS kasutab oma **failhaldussüsteemi** (_File Management System_ - **FMS** ).
...
### 2.2.2 Andmebaasisüsteemi komponendid: andmefailid, andmesõnastik, indeksid, statistilised andmed
...
### 2.2.5 Andmebaasi kasutaja, koostaja, programmeerija ja administraatori rollid
...
a) suurendab faili kopeerimise kiirust

b) võimaldab faili paremini jagada kettasektorite vahel

c) võimaldab kindlamat üheaegset failikasutust X

d) suurendab üksikute baitide kirjutamise kiirust

2. Miks on andmehalduse seisukohalt kasulikud indeksid?

a) suurendab andmete kopeerimise kiirust

b) võimaldab kergemini kätte saada binaarandmeid

c) aitab soovitud tunnuse järgi kiiremini otsida X

d) võimaldab kindlamat üheaegset failikirjutust

## 2.3 Andmete modelleerimine
...
**Füüsiline vaade** (\_physical level)\_tegeleb andmete salvestamise viisidega. Seda võib lihtsalt saavutada, delegeerides selle ülesande operatsioonisüsteemi failihaldussüsteemile või otsese ja optimaalse kettaseadmete haldusega.

**Loogiline vaade** (_conceptual level)_ käsitleb, kuidas informatsioon struktureeritakse, defineerides seosed andmete vahel.

**Kasutaja vaade** (_view level)_ käsitleb kasutajate juurdepääsu infole nii võimalike operatsioonide kui ka kasutajale kuvatava info struktuuri kontekstis. Lihtsamate rakenduste puhul võib see suuresti sarnaneda loogilisele vaatele, kuid kasutajamugavuse huvides saab kasutajavaadet vajadust mööda kohandada.
...
**Lisamaterjal abstraktsete andmete kohta:** http://en.wikipedia.org/wiki/Abstract\_data\_types (artikkel wikipedias).

### 2.3.2 Andmemudelite grupid
...
Esimene objektidega seotud mudel sai nimeks olemi-seosemudel või **objekti-seosemudel** _(entity/relationship (E/R) model)._ Mudel algab objekti (olemi, andmeühiku) määratlusega. Näiteks personalihalduse kontekstis oleks objektiks või andmeühikuks töötaja. Objektide gruppi (ettevõtte töötajad) nimetatakse **objektide hulgaks** (või andmeühikute hulgaks _–_ _entity set_). Igat objekti iseloomustavad atribuudid ehk omadused _(attributes)\_\_,_ millised teda kirjeldavad. Töötaja omaduseks (atribuudiks) on näiteks tema töölevõtmise kuupäev.
...
- kapseldus _( **encapsulation** )_ või võime eraldada välised aspektid (funktsionaalsus) sisemisest esitlusest, kuna see pole ülemiste rakenduse kihtide seisukohalt oluline, on ta maskeeritud.

Mõlemad toodud aspektid võimaldavad selgitada (kasutada) abstraktse andmetüübi _(\_\_abstract data types (ADT)_) mõistet.

Objekte iseloomustavad atribuudid (andmed) ja tegevused (kuidas objekte on võimalik manipuleerida). Näiteks &quot;tekst&quot; tüüpi objektil on võimalik defineerida teksti lisamise operatsioon tekstide liitmise teel või mingi lõigu otsimine sellest tekstist. Sarnaselt olemite või olemite hulkadega, võib objekt kuuluda objektide **klassi**. On samuti võimalik määratleda, et objektid on ainsad klassi **eksemplarid** (või **esindajad** ), mis esindavad mingit tuntud rakendust. Igal objektil on oma **tunnus** (_OID_ _–_ _object identifier_), mis on unikaalne kogu süsteemi ulatuses ja mis kujutab endast viidet objektile enesele.
...
b – Inimeste turismireiside sihtkohad

c - Inimeste sünnilinnad X

d – Inimese isikukood

-

1. Andmete kontseptuaalne tase on seotud põhiliselt

a – kõvaketta sektoritega

b – failisüsteemiga

c – andmebaasi struktuuriga X

d – kasutaja vaatega

## 2.4 Relatsiooniline mudel
...
### 2.4.2 Tähtsamad relatsioonilise mudeli terminid: relatsioon, võti, primaarvõti, alternatiivne võti, võõrvõti, viiteterviklus
...
### 2.4.3 Näide andmete normaliseerimiseks esimesele, teisele ja kolmandale normaalkujule
...
**2.4.4 Kordamisküsimused**

1. Mida aitab viiteline terviklus?

a – Hoolitseb, et ei oleks korduvaid andmeid

b – Hoolitseb, et seotud andmetest ei saaks üht osa kogemata kustutada X

c – Hoolitseb, et igal real oleks primaarvõti

d – Piirab ühe kirje andmemahu kilobaitides

2. Milline neist on kolmandal normaalkujul olevate andmete eritunnus?

a – Ühel real olevad andmed ei sõltu üksteisest X

b – Ühes lahtris on üks väärtus

c – Ühte tabeli lahtrisse võib paigutada rohkem kui ühe väärtuse

d – Igas tabelis on rohkem kui üks primaarvõti

## 2.5 Päringukeeled

Selle alateema materjale läbi töötades õpid tundma mitmesuguste päringukeelte ülesehitust, võimalusi ning kasutuskohti.

### 2.5.1 Protseduuriliste ja mitteprotseduuriliste päringukeelte eristamine
...
### 2.5.3 Andmete määratluskeel (DDL) ja andmede manipuleerimiskeel (DML)
